...
from sistema.app.models import FormsSubmission, User
...
def test_admin_stream_requires_valid_key():
    with TestClient(app) as client:
...
def test_list_and_remove_registered_user():
    with TestClient(app) as client:
        ...
        checkout_before = client.get("/api/admin/checkout", headers={"x-admin-key": "admin-test-key"})
        assert checkout_before.status_code == 200
        assert all(row["rfid"] != "USERDEL1" for row in checkout_before.json())

        checkin_before = client.get("/api/admin/checkin", headers={"x-admin-key": "admin-test-key"})
        assert checkin_before.status_code == 200
        assert all(row["rfid"] != "USERDEL1" for row in checkin_before.json())

        events_before = client.get("/api/admin/events", headers={"x-admin-key": "admin-test-key"})
        assert events_before.status_code == 200
        assert any(event["rfid"] == "USERDEL1" for event in events_before.json())

        users_before = client.get("/api/admin/users", headers={"x-admin-key": "admin-test-key"})
        ...
        remove_user = client.delete("/api/admin/users/USERDEL1", headers={"x-admin-key": "admin-test-key"})
        assert remove_user.status_code == 200

        users_after = client.get("/api/admin/users", headers={"x-admin-key": "admin-test-key"})
        assert users_after.status_code == 200
        assert all(row["rfid"] != "USERDEL1" for row in users_after.json())

        checkout_after = client.get("/api/admin/checkout", headers={"x-admin-key": "admin-test-key"})
        assert checkout_after.status_code == 200
        assert all(row["rfid"] != "USERDEL1" for row in checkout_after.json())

        checkin_after = client.get("/api/admin/checkin", headers={"x-admin-key": "admin-test-key"})
        assert checkin_after.status_code == 200
        assert all(row["rfid"] != "USERDEL1" for row in checkin_after.json())

        events_after = client.get("/api/admin/events", headers={"x-admin-key": "admin-test-key"})
...
def test_inactive_users_are_listed_by_highest_inactivity_and_excluded_from_checkin_checkout(monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        inactive_rows = client.get("/api/admin/inactive", headers={"x-admin-key": "admin-test-key"})
...
def test_archive_events_creates_csv_clears_table_and_lists_downloads(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        archives_list = client.get("/api/admin/events/archives", headers={"x-admin-key": "admin-test-key"})
...
def test_delete_archived_event_csv(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        delete_res = client.delete(f"/api/admin/events/archives/{file_name}", headers={"x-admin-key": "admin-test-key"})
        assert delete_res.status_code == 200
        assert delete_res.json()["ok"] is True

        list_res = client.get("/api/admin/events/archives", headers={"x-admin-key": "admin-test-key"})
        assert list_res.status_code == 200
        assert all(item["file_name"] != file_name for item in list_res.json()["items"])

        missing_res = client.get(f"/api/admin/events/archives/{file_name}", headers={"x-admin-key": "admin-test-key"})
...
def test_archive_events_without_current_rows_returns_existing_archives(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        archive_first = client.post("/api/admin/events/archive", headers={"x-admin-key": "admin-test-key"})
        assert archive_first.status_code == 200
        assert archive_first.json()["created"] is True

        archive_second = client.post("/api/admin/events/archive", headers={"x-admin-key": "admin-test-key"})
...
def test_list_event_archives_supports_backend_filter_and_pagination(tmp_path):
    ...
    with TestClient(app) as client:
        paged = client.get("/api/admin/events/archives?page=2&page_size=5", headers={"x-admin-key": "admin-test-key"})
        ...
        filtered = client.get("/api/admin/events/archives?q=2026-03-01&page=1&page_size=5", headers={"x-admin-key": "admin-test-key"})
